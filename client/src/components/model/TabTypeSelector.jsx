...
export function TabTypeSelector({ tabType, setTabType }) {
    return (
        <div>
            <Label className="block text-sm font-medium leading-6 text-gray-900">Select Tab Type</Label>
            <Listbox value={tabType} onChange={setTabType}>
                <div className="relative mt-2">
                    <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-teal-500 sm:text-sm sm:leading-6">
                        <span className="block truncate">{tabType.name}</span>
                        ...
                        {tabTypes.map((type) => (
                            <Listbox.Option
                                ...
                                className={({ active, selected }) =>
                                    `relative cursor-default select-none py-2 pl-3 pr-9 ${active ? 'bg-teal-400 text-white' : 'text-gray-900'
                                    }`
                                }
                            >
                                {({ selected }) => (
